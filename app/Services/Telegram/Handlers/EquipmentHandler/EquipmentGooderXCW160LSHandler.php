...
class EquipmentGooderXCW160LSHandler implements CommandsInterface
{
            ...
            'keyboard' =>
                [
                    [ //строка
                        ...
                        */
                        [ //кнопка
                            'text' => '🎬 Відео інструкція XCW-160LS',
                        ],
    ...
    public function handle(string $message, int $senderId, string $fileName, int $callback, int $mediaGroupId): MessageDTO
    {
        $this->client->post(
            ...
            [
                'json' => [
                    'chat_id' => $senderId,
                    'media' => [
                        ['type' => 'photo', 'media' => config('messenger.telegram.ngrok').'/'. 'GooderXCW160LS/GooderXCW160LS-1.jpg'],
                        ['type' => 'photo', 'media' => config('messenger.telegram.ngrok').'/'. 'GooderXCW160LS/GooderXCW160LS-2.jpg'],
                        ['type' => 'photo', 'media' => config('messenger.telegram.ngrok').'/'. 'GooderXCW160LS/GooderXCW160LS-3.jpg'],
                    ],
                ],
            ]
        );
        ...
        $message .= 'Характеристики Gooder XCW-160LS.'.PHP_EOL.PHP_EOL;
        $message .= 'Обсяг,л: 160'.PHP_EOL;

        $message .= 'Потужність, кВт — 0,23'.PHP_EOL;
        $message .= 'Напруга, В — 220'.PHP_EOL;
        $message .= 'Розмір, мм — 874х568х686'.PHP_EOL;
        $message .= 'Вага, кг — 71'.PHP_EOL;
        ...
        $message .= 'Вартість оренди 2200грн/міс';
