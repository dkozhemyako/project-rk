...
class ClientAddressStreetHandler implements AgreementInterface
{
    public const AGR_STAGE_CLIENT_STREET = '_CLIENT_STREET';


    public function handle(AgreementDTO $agreementDTO, Closure $next): AgreementDTO
    {
        $key = $agreementDTO->getSenderId() . self::AGR_STAGE_CLIENT_STREET;

        if (Redis::get($agreementDTO->getSenderId()) == 14
            && $agreementDTO->getMessage() == TelegramCommandEnum::agreementBack->value)
        {
            Redis::del(
                $agreementDTO->getSenderId() . ClientAddressHouseHandler::AGR_STAGE_CLIENT_HOUSE,
            );
            Redis::set($agreementDTO->getSenderId(), 13);

            $agreementDTO->setMessage(
                '💬 Вкажіть номер будинку Вашої прописки.'
