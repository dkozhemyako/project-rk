...
class EquipmentAddressTownHandler implements AgreementInterface
{
    ...
    public function handle(AgreementDTO $agreementDTO, Closure $next): AgreementDTO
    {
        ...
        if (Redis::get($agreementDTO->getSenderId()) == 18
            && $agreementDTO->getMessage() == TelegramCommandEnum::agreementBack->value)
        {
            Redis::del(
                $agreementDTO->getSenderId() . EquipmentAddressStreetHandler::AGR_STAGE_EQUIP_STREET,
            );
            Redis::set($agreementDTO->getSenderId(), 17);
